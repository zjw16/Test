module.exports = app => {
    ...
    router.get('/news/init', async (req, res) => {
        ...
        const cats = await Category.find().where({
            parent: parent
        }).lean() // lean()表示取json js对象 数组
        const newsTitle = ["6月10日体验服停机更新公告", "6月10日外挂专项打击公告", "6月10日“演员”惩罚名单", "6月10日净化游戏环境声明及处罚公告", "【优化预告】游戏内更新机制优化", "【优化预告】游戏内更新机制优化", "国货生而荣耀，今晚来《新国货》一起上分可好？", "选“王者零距离”举办城市，赢【全英雄全皮肤】大礼包！", "你玩了假魂斗罗？ 真魂斗罗6.6三周年庆开启", "策划有话说丨出装可以预设12件？bp时的套装推荐是什么？", "6月10日体验服停机更新公告", "6月10日外挂专项打击公告", "6月10日“演员”惩罚名单", "6月10日净化游戏环境声明及处罚公告", "6月8日体验服停机更新公告", "应援KPL春决得好礼，上官婉儿-天狼绘梦者即将开售", "【破浪前行吧英雄们】活动开启公告", "参与活动免费解锁KPL限定皮肤个人专属购买6折特权", "新英雄蒙恬上架，多重好礼等你解锁", "应援KPL得好礼，福利清单就绪", "虎牙明星主播踢馆名校战队，峡谷高材生与学霸的荣耀对决", "2020年KPL春季赛常规赛最佳阵容及最佳选手评选方式公布", "2020年KPL春季赛季后赛赛程赛制公布，5月28日16:00热血开战", "【原创内容大赛音乐比赛】优秀作品合集（二）", "大众赛事合作赛道全面开启，携手合作伙伴共建王者电竞生态"]
        const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5) // 从cats中复制一份数组出来进行排序
        ...
        await Article.deleteMany({}) // 传递一个空对象 以任意条件去查询 删除
        await Article.insertMany(newsList) // 插入newsList
        res.send(newsList)
    })
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([{
            ...
            }, // 条件查询 类似于mySQL中的where
            {
                $lookup: {
                    from: 'articles', // 根据模型中数据 需要写成小写复数形式
                    localField: '_id',
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([{
            ...
            {
                $lookup: {
                    from: 'heroes', // 根据模型中数据 需要写成小写复数形式
                    localField: '_id',
